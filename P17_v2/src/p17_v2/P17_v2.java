...
public class P17_v2 {
        ...
        do {
            ...
            switch (option) {
                ...
                case 9:
                    ...
                    break;
                case 10:
                    System.out.println("American Standard Code for Information Interchange. Is a character encoding standard for electronic communication and represents text in computers, telecommunications equipment, and other devices.");
                       for(int c=0; c<=255 ; c++ ) {                     
                           System.out.println(c+ "\t " + Integer.toString(c, 16) +"\t" + (char)c);  
                       }  
    ...
    private static String exercise8(String name) {//primero lo que devuelve y el segundo String como lo recibe

        String result="";  //pilla letra por letra para pasarlo a binario 
        char ch;
        for (int i = 0; i < name.length(); i++)//mide automáticamente la extension del texto input, en este caso 5 porque la palabra es jordi
        {
            ch = name.charAt(i);//se refiere a cada carácter en cada posicion que está escrita. 
            result += Integer.toBinaryString(ch) + " ";//con += va acumulando datos
        }
    ...
    private static String exercise9(int numberex9) {
        ...
        String resultex9 = Integer.toBinaryString(numberex9);
        return resultex9;
    }
}
